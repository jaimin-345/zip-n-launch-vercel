...
export const Step6_PatternAndLayout = ({ formData, setFormData, associationsData = [], stepNumber = 5 }) => {
  ...
  const handleSaveStaffContact = async () => {
    ...
    try {
      ...
      // Update formData
      setFormData(prev => {
        const newOfficials = [...(prev.officials || [])];
        if (newOfficials[editingStaff.index]) {
          newOfficials[editingStaff.index] = {
            ...newOfficials[editingStaff.index],
            name: currentName,
            email: currentEmail,
            phone: currentPhone
        ...
        Object.keys(newAssociationJudges).forEach(assocId => {
          ...
          if (assocData?.judges) {
            ...
            newAssociationJudges[assocId] = {
              ...assocData,
              judges: assocData.judges.map((judge) => {
                ...
                if (judge.name && judge.name.toLowerCase().trim() === originalJudgeName.toLowerCase().trim()) {
                  return { ...judge, name: currentName, email: currentEmail, phone: currentPhone };
                }
                return judge;
              })
            };
