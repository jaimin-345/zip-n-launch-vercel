            ...
            {steps.map((step, index) => {
                ...
                return (
                    <React.Fragment key={step.id}>
                        <div className="flex flex-col items-center text-center w-20 cursor-pointer" onClick={() => handleStepClick(step.id)}>
                            <div className={cn(
                        ...
                        {index < steps.length - 1 && (<div className={`flex-1 h-1 mt-5 mx-2 rounded-full transition-colors duration-300 ${completedSteps.has(step.id) && completedSteps.has(step.id + 1) ? 'bg-green-600' : (currentStep > index + 1 ? 'bg-primary' : 'bg-border')}`} />)}
                    </React.Fragment>
                )
            })}
        </div>
